...
public:
	...
	Counter(int minVal, int maxVal, int initVal = 0) : Frame() {
		...
		setSize(ivec2(FRAME_ROUNDED_RADIUS * 2 + COUNTER_INDICATOR_WIDTH * 2 + displayButton->getSize().x, displayButton->getSize().y));
		UIElem::addChild(displayButton); //don't offset it
		displayButton->setPosition(ivec2(pos.x + size.x / 2 - displayButton->getSize().x / 2, pos.y)); //offset it here
		...
		plusIndicator->setPosition(pos + ivec2((FRAME_ROUNDED_RADIUS + COUNTER_INDICATOR_WIDTH) / 2 - textWidth / 2, size.y / 2 - textHeight / 2));
		
		textWidth = minusIndicator->getSize().x;
		textHeight = minusIndicator->getSize().y;
		minusIndicator->setPosition(pos + ivec2(FRAME_ROUNDED_RADIUS + COUNTER_INDICATOR_WIDTH + ((Button*)children.front())->getSize().x + (COUNTER_INDICATOR_WIDTH + FRAME_ROUNDED_RADIUS) / 2 - textWidth / 2, size.y / 2 - textHeight / 2));
	...
	/*void draw(int depth)
		...
		ivec2 renderAt = pos + ivec2((FRAME_ROUNDED_RADIUS + COUNTER_INDICATOR_WIDTH) / 2 - textWidth / 2, size.y / 2 - textHeight / 2);
		TextRender::render(plus, renderAt.x, renderAt.y); //overlay value text

		TextRender::metrics(minus, &textWidth, &textHeight);
		renderAt = pos + ivec2(FRAME_ROUNDED_RADIUS + COUNTER_INDICATOR_WIDTH + ((Button*)children.front())->getSize().x + (COUNTER_INDICATOR_WIDTH + FRAME_ROUNDED_RADIUS) / 2 - textWidth / 2, size.y / 2 - textHeight / 2);
